...
test("briefings preserve unseen memory, open questions and source provenance", () => {
  ...
  assert.match(briefing.briefing ?? "", /APPROVED UNSEEN COMPANY MEMORY/);
  assert.match(briefing.briefing ?? "", /Annual discount is unknown/);
  assert.match(briefing.briefing ?? "", /Pricing memo · chunk 3/);
  assert.match(briefing.briefing ?? "", /clinic partner/);
  assert.equal(briefing.contextVersion, 5);
});
